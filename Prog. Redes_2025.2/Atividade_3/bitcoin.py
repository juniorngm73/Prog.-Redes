...
def findnounce(data_To_Hash, bitsToBeZero, start_nonce=0):      #crio a função findnounce com tres parametros: data_to_hash(via pegar a frase, bitsToBezero(vai pegar o núm
    print(f"Iniciando a busca por hash de '{data_To_Hash}' que inicia com {bitsToBeZero} zeros ")
    ...
    while True:
        ...
        if hashtextobinario.startswith("0"*bitsToBeZero): #a string começa a partir do numero que eu defino no bitsToBeZero quando encontrado o valor eu paro com o break
            break
        start_nonce += 1
