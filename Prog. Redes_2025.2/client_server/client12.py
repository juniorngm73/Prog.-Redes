...
try:
    ...
    # UPLOAD 
        ...
        tcp_socket.send(len(nome_bytes).to_bytes(4, 'big'))
        tcp_socket.send(nome_bytes)
        print(f" Enviado para o servidor: 'Upload_Arquivo:{nome_arquivo}'")

        if tcp_socket.recv(1) == b'\x01':
            tam_arquivo = os.path.getsize(nome_arquivo)
            print(f" Preparando para enviar arquivo: {nome_arquivo}, Tamanho: {tam_arquivo} bytes.")
            ...
            print(f" Upload de '{nome_arquivo}' completo. Total de {enviado} bytes enviados.")
            if tcp_socket.recv(1) == b'\x01':
